...
# TODO(toabctl): For now we just use the first ml2_type_driver for floating network type
#                But this should be selectable with an attribute and maybe in the UI
ml2_type_driver_first = node[:neutron][:ml2_type_drivers][0]
networking_plugin = node[:neutron][:networking_plugin]
case networking_plugin
when 'ml2'
  case ml2_type_driver_first
  when 'vlan'
  ...
  when 'gre'
    ...
    Chef::Log.error("ml2 type driver '#{ml2_type_driver_first}' invalid for creating provider networks")
  end
when 'vmware'
  fixed_network_type = ""
